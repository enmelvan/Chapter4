...
otu.nsample(my_class)
names(otu.nsample(my_class))[otu.nsample(my_class)>=142/2]


# Drew: I pruned all but the 4 most frequently observed taxa
taxon_sub <- prune_taxa(names(otu.nsample(my_class))[otu.nsample(my_class)>=142/2],my_class)
...
diagnoseFit(lean.fit, mySubset, annotate = TRUE) # R^2 > 0.75
showInteraction(lean.fit,mySubset)
diagnoseBiomass(lean.fit)                             # looks strange, periodic fluctuations
...
diagnoseBiomass(obese.fit)                             # convergence looks o.k.
