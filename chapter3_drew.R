...
# Drew: I think the key issue is the number of samples
# with the taxon present, not the mean abundance, so
# I calculate number of samples with each OTU present.
# There are four frequently observed OTUs:
# Otu0001,Otu0044,Otu0076,Otu0339
nsample <- c()
for(i in 1:nrow(my_phylum_rand@otu_table@.Data)) {
  temp <- my_phylum@otu_table@.Data[i,]
  nsample[i] <- length(temp[temp>0])
}
names(nsample) <- row.names(my_phylum_rand@otu_table@.Data)
nsample


# Drew: I pruned the 4 infrequently observed taxa
taxon_sub <- prune_taxa(names(nsample[nsample>=131]),my_phylum)


# Drew: You should add poisson noise to counts, not proportions.
# I created a new object for the noisy data and updated the counts directly, 
...
#TRANSFORM TO RELATIVE ABUNDANCE. Drew: I didn't change your code here,
# but please note that, since I am taking this step after pruning taxa and
# creating random data, the function make_relative() in the library funrar
# is no longer needed.
...
# Drew: I never needed to uese these convenience functions. I just 
# extracted the abundance data directly below.
...
#Lean analysis (works fine; converges after 30 iterations)
...
lean.fit <- func.EM(mySubset,mySubset_rand)
diagnoseFit(lean.fit, mySubset_rand, annotate = TRUE) # R^2 > 0.9 (nice!)
showInteraction(lean.fit,mySubset_rand)
diagnoseBiomass(lean.fit)                             # convergence looks good


#obese analysis (works fine; converges after 30 iterations)
...
obese.fit <- func.EM(mySubset,mySubset_rand)
diagnoseFit(obese.fit, mySubset_rand, annotate = TRUE) # R^2 > 0.9 (nice!)
